...
class TestEvaluator:
    ...
    def get_accuracy(self, ans_sheet=None) -> float:
        ans_sheet = self.answers.all() if ans_sheet is None else ans_sheet
        total_answerable = self.total_answerable(ans_sheet)
        total_mark = self.get_total_mark(ans_sheet)
        accuracy = total_mark / total_answerable if total_answerable > 0 else 0
        return accuracy * 100
    ...
    def get_chapter_wise_short_report(self):
        ...
        report = [{
            ...
            'accuracy': self.get_accuracy(chapter_wise_ans_sheet[chapter]['answers']),
    ...
    def suggestions(self):
        ...
        while len(reports) > 0 and len(sug) < 5:
            if reports[0]['accuracy'] < 100:
                sug.append(reports[0])
            reports.pop(0)
        return sug
