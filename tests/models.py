...
class Test(models.Model):
    ...
    def question_count(self):
        count = 0
        for member in self.question_set.members.all():
            count += member.metadata.get_question_count()
        return count
    
    @property
    def total_marks(self):
        return int(self.question_count * self.marking_criterion.correct)
    
    @property
    def default_test_time(self):
...
class TestParticipant(models.Model):
    ...
    def has_ended(self):
        has_ended = False
        if self.has_timer: has_ended = self.test.timer.has_ended() 
        return has_ended or self.has_submitted
        ...
        for member in self.test.question_set.members.all():
            ...
            if questions:
                answer_paper.append({
                    ...
                    'appearance_label': meta.appearance.label,
                    'chapter': meta.chapter.name,
